...
interface CommentSectionProps {
  postId: string;
  onCommentChanged?: () => Promise<void> | void;
}
...
const CommentSection: React.FC<CommentSectionProps> = ({ postId, onCommentChanged }) => {
  ...
  const fetchComments = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      // Force clear previous local state before fetching new data
      setComments([]);
      setLikedComments({});
      setCommentLikes({});
      ...
      // Sort parent comments by most recent
      ...
      const fetchLikes = async () => {
        ...
        for (const comment of parentComments) {
          try {
            ...
            likedMap[comment.id] = likesData.userLiked;
            likesCountMap[comment.id] = likesData.count;
            ...
            if (comment.replies && comment.replies.length > 0) {
              for (const reply of comment.replies) {
                try {
                  ...
                  likedMap[reply.id] = replyLikesData.userLiked;
                  likesCountMap[reply.id] = replyLikesData.count;
                } catch (error) {
  ...
  const handleReply = async (parentId: string) => {
    ...
    try {
      ...
      // Update the UI optimistically without full refresh
      setComments(prevComments =>
      ...
      // Do NOT call fetchComments() here to avoid full refresh
      // Only update the like counts separately if needed
      const fetchJustLikes = async () => {
        try {
          const likesData = await getCommentLikes(response.id);
          setLikedComments(prev => ({ ...prev, [response.id]: likesData.userLiked }));
          setCommentLikes(prev => ({ ...prev, [response.id]: likesData.count }));
        } catch (error) {
          console.error("Error fetching likes for new reply:", error);
        }
      };
      
      // Fetch likes for the new reply in background
      fetchJustLikes();
      
      // Update comment count in parent component
      if (onCommentChanged) {
        // Now we DO need to await this to ensure count is updated
        await onCommentChanged();
      }
  ...
  const handleLikeComment = async (commentId: string, e?: React.KeyboardEvent) => {
    ...
    try {
      ...
      // Don't call fetchComments() as it causes a full refresh
      // Instead rely on the optimistic updates we just made
      
      if (onCommentChanged) {
        // Don't await this to prevent parent component from refreshing unnecessarily
        onCommentChanged();
      }
    } catch (error) {
      ...
      // In case of error, refresh to get correct state
      await fetchComments();
    }
  };
  ...
  const formatTimeWithFallback = (timeValue: any) => {
    try {
      if (!timeValue) return 'some time ago';
      
      // Handle Firestore Timestamp
      if (timeValue && typeof timeValue === 'object' && 'seconds' in timeValue) {
        return formatDistanceToNow(new Date(timeValue.seconds * 1000), { addSuffix: true });
      }
            ...
            <div className="comment-info">
              ...
              <div className="comment-metadata">
                <span className="comment-time">
                  {comment.editedAt ? (
                    <>edited {formatTimeWithFallback(comment.editedAt)}</>
                  ) : (
                    formatTimeWithFallback(comment.createdAt)
                  )}
                </span>
              </div>
            </div>
